dat <- read.csv(file = "BSP_design_jittered.csv");   # Gaussian perturbation.
dat <- dat[-1];
pblh <- dat[,5];
...
for(i in 1:length(pblh)){
  print(paste0("Jittering sample ",i))
  for(j in 1:num_rep){
    tmp <- pblh[i] + rnorm(1,mean = 0, sd = jp);
    qpbl <- comp_qpb(tpotpbl = dat[i,4],lcw = dat[i,3],pblh = tmp);
    permissable <- TRUE;
    
    if(!(1 < dat[i,1] & dat[i,1] < qpbl)) permissable <- FALSE;
    if(!(1 < dat[i,2] & dat[i,2] < 15))                         permissable <- FALSE;
    if(!(80 < tmp & tmp < 3000))                      permissable <- FALSE;
    if(!(dat[i,3] > 0.1))                                     permissable <- FALSE;
    if(!(is_cloudy(qpbl,dat[i,4],tmp)))                  permissable <- FALSE;
    if(permissable) pblh[i] <- tmp;
    print(permissable)
  }
}
  ...
  write.csv(dat,file = "BSP_design_jittered.csv")
