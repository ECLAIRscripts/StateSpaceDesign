#parameters
num_samples <- 150;
setwd("~/Documents/Main_project_FMI/ECLAIR_files/State_space_design/Day-Night_designs/lvl4/day")
#--------------------------


allpoints <-read.csv("eclair_dataset_aerosols.csv",header=TRUE)
...
ks <- as.vector(allpoints[,"ks"])
as <- as.vector(allpoints[,"as"])
cs <- as.vector(allpoints[,"cs"])
rdry_AS_eff <- as.vector(allpoints[,"rdry_AS_eff"])
cos_mu <- as.vector(allpoints[,"cos_mu"]) 

#length(which(pbl>300))/length(pbl)
...
as <- as[sel_ind]
cs <- cs[sel_ind]
rdry_AS_eff <- rdry_AS_eff[sel_ind]
cos_mu <- cos_mu[sel_ind]
...
#"q_inv","tpot_inv","lwp","tpot_pbl","pblh","num_pbl", "cos_mu"
IN_MAT <- as.matrix(cbind(q_inv,tpot_inv,lwp,tpot_pbl,pbl,ks,as,cs,rdry_AS_eff,cos_mu));
...
#IN_MAT <- as.matrix(cbind(q_inv,tpot_inv,lwp,tpot_pbl,pbl,B_AS,rdry_AS,rdry_AS_eff,cdnc));

  colnames(DESIGN) <- c("q_inv","tpot_inv","lwp","tpot_pbl","pblh","ks","as","cs","rdry_AS_eff","cos_mu")
