...
IN_MAT <- as.matrix(cbind(q_inv,tpot_inv,lwp,tpot_pbl,pbl,cdnc,cos_mu));
...
#as.matrix(cbind(q_inv,tpot_inv,lwp,tpot_pbl,pbl,cdnc,cos_mu));
...
#IN_MAT <- as.matrix(cbind(q_inv,tpot_inv,lwp,tpot_pbl,pbl,cdnc));
  ...
  colnames(DESIGN) <- c("q_inv","tpot_inv","lwp","tpot_pbl","pblh","cdnc","cos_mu")
