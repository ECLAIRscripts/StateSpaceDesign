...
for(i in 1:nrow(IN_MAT)){
  ...
  #print(val)
  permissable <- TRUE;
  if(!(1 < val[6] & val[6] < h2o_mean[i])) permissable <- FALSE;
...
for(i in 1:length(BSP)){
  ...
  qpbl_list[i] <- h2o_mean[ind];
  #print(IN_MAT[BSP[[i]][ind],])
}

for(i in 1:length(BSP)){
  print(paste0("ECHAM qpbl , ",qpbl_list[i]," calculated qpbl, ",comp_qpb(tpotpbl = SAMPLES[[i]][4],lcw = SAMPLES[[i]][3],pblh = SAMPLES[[i]][1])));
}
